const userdetails = [
  ...
  var rows = 10;
  var currentpage = 1;
  var elements = document.getElementById("myTable");
  var pagination = document.getElementById("pagination");

  displayelements(userdetails,elements,rows,currentpage);

  function firstpage(x) {
    currentpage = x;
    displayelements(userdetails,elements,rows,currentpage);
  }
  ...
  function prevpage() {
    if (currentpage <= 1) {
    window.alert("You are in the first page")
  ...
  function forward(x) {
    if (currentpage >= 10) {
      window.alert("You've Reached last page")
    } else currentpage = currentpage + 1
      displayelements(userdetails,elements,rows,currentpage);
    } 
  ...
  function displayelements(items, wrapper, rows_per_page, page) {
    ...
    let end = loopstart + rows_per_page;
    var paginateditems = items.slice(loopstart, end);
    paginateditems.reverse();
      for (let i=0; i < paginateditems.length; i++ ) {
        ...
        cell1.innerHTML = `${paginateditems[i].id}`;
        
        var cell2 = row.insertCell(1);
        cell2.innerHTML = `${paginateditems[i].name}`;
        
        var cell3 = row.insertCell(2);
        cell3.innerHTML = `${paginateditems[i].email}`;
      
      }   


    // function pageblock(wrapper,rows_perpage,page,items) {
    //   noofpages = Math.ceil(items.length / rows_perpage );
    //     for (i =0; i<=noofpages; i++) {
    //       let btn = document.createElement("button");
    //       btn.innerText = page;
    //       btn.className = "btn btn-info btn-pagination";
    ...
    // pageblock(pagination,rows,currentpage,userdetails)
  }
